...
public class UserServiceImpl implements UserService {
	 ...
	 * @return int
	 * 
	 * @throws MakeBarcodeException when generate barcode image error
	 */
	public int signUp(String id, String email, String password, String name,
		...
		int srl = userDao.getMaxSeq()+1;
		
		// sign up process
		if (makeBarcode(id) == GlobalVariables.BARCODE_MAKE_SUCCESS) {
			UserModel user = new UserModel(srl, id, email, password, name, nick_name, phoneNum);
	...
	/** check ID is already in db or not
	 * 
	 * @param id
	 * 			: id
	...
	/** check E-MAIL & Phone Number is already in db or not
	 * 
	 * @param phoneNum, email
	 * 			
	 * @return boolean - true or false
	 */
	public boolean checkDupPhoneEmail(String phoneNum, String email){
		UserModel user = userDao.getUserByEmail(email);
		UserModel user2 = userDao.getUserByPhoneNum(phoneNum);
